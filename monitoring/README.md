...
服务从 PostgreSQL 自动发现账户及其真实分组关系，优先分析已有请求；只有记录到渠道错误时，才对支持的账户发送最小化恢复探测，复用一份账户结果推断所有所属分组状态，并将监控历史写入自己的 `monitoring_*` 表。分组不会重复发送模型请求：分组状态结合账户优先级、`account_groups.priority` 和近 24 小时真实请求量，模拟网关最可能使用的候选层级。面板和用量统计展示启用账户、处于错误状态的账户及 `status=active` 的分组；未分组和停用分组不会生成展示对象。错误账户保留为红色诊断对象，但不进入启用统计或分组候选；只有同时存在渠道错误证据时才进入恢复探测。探测失败不会修改 `accounts.status`、`accounts.schedulable` 或任何网关路由状态。
...
- 真实请求成功或主动验证成功都会结束当前失败影响并生成恢复证据/告警；如果没有用户请求，只有错误触发的恢复探测能确认“已恢复”，面板会标明确认来源。只有旧探测失败而没有新的渠道错误时，面板显示“等待渠道证据”，并明确当前不发送上游请求。
- 账户配置或状态更新会使更早的请求和验证结果失效，但不会因此自动发送探测；必须等新的真实渠道错误。错误账户保留为红色诊断对象，但不进入启用分组及健康统计。
- 监控只主动探测账户，不对分组重复发请求；一个账户的结果会供它所属的所有启用分组复用。活动分组只有同时存在启用渠道和可调度账户时才可路由，否则面板直接显示“失败：无启用渠道或可调度候选”。停用分组和未分组不展示、不参与分组健康告警。
- 分组按路由层级判定：账户优先级先于分组绑定优先级；高优先级候选异常但仍有可用候选时保留风险说明，健康颜色仍只按可用候选的响应延迟归入“可用”或“可用但延迟高”（失败候选的超时不拖慢仍可用的渠道）。近 24 小时真实请求量会覆盖无流量时的先验权重，避免低流量失败账户把整个组误报为故障；所有候选均失败才归入“错误/不可用”，仍有未验证候选时不会武断判定全组失败。
- 一次真实请求成功证明分组当前存在可用路径；如果候选检查同时存在异常，卡片说明保留该风险，但不额外制造第四种健康状态。单次聚合失败但尚无新的真实成功证据时仍按内部确认策略处理；只有候选检查连续失败达到告警阈值，且之后没有新的真实成功请求时，外层才显示“错误/不可用”；历史记录仍保留原始候选检查结果。
...
- 最近 24 小时的 24 段状态轨迹（真实请求优先、错误恢复探测补证据；无新请求时继续采用最近有效状态）。面板只用三种健康颜色：绿色表示可用且延迟低，黄色表示仍可用但延迟高（成功请求总耗时达到 20 秒），红色表示错误/不可用；空档不会作为第四种健康状态显示。
