...
- 24 小时、7 天、30 天健康窗口；
- 分组、账户筛选；
- 所选健康窗口内的窗口观测通过率、首字/首字节中位数，以及成功样本的最快、中位数、P95 延迟；
...
面板的 `24 小时 / 7 天 / 30 天` 选择会同时控制窗口观测通过率、首字中位数、最快、中位数、P95 和 24 段状态轨迹；状态徽标及卡片底部的“最近验证”则使用最新有效证据，不受统计窗口限制。
...
支持 `1h`、`24h`、`today`、`7d`、`15d`、`30d`。返回启用账户/分组范围内的请求数、Tokens 总量、总成本、每百万 Tokens 成本、包含渠道明细的小时/天时间桶，以及账户、模型和分组排行；模型和分组排行同时覆盖按 Tokens、成本与每百万 Tokens 成本的前列对象，账户和分组还附带缓存命中率。
