...
- 账户、分组用量卡片可按平台筛选，并可切换展示四类 Tokens、原始/实际成本、有效倍率、缓存命中率与每百万 Tokens 实际成本；卡片支持按使用成本、每百万 Tokens 成本、有效倍率或总 Tokens 升序/降序排列，账户视图另支持按账户优先级排列（数值越小越优先），默认按每百万 Tokens 成本升序；筛选只影响账户/分组卡片，KPI、环图和趋势仍按完整时间窗口统计，其他平台会动态生成按钮。
