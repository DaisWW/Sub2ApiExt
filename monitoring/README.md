...
- 分组状态徽标、通过率和 24 段轨迹使用 `monitoring_checks.source = 'aggregate'` 的账户聚合检查；分组历史明细只展示原始请求及其实际经由账户，避免把分组健康记录或当前成员列表误当成历史请求归属。
...
- 最近 1 小时内的窗口观测通过率、首字/首字节最快和中位数，以及成功样本的完整请求总耗时最快、中位数、P95 延迟；
...
面板卡片健康统计固定使用最近 1 小时，控制通过率、首字最快、首字中位数、完整请求总耗时最快、中位数和 P95；状态轨迹固定使用最近 24 小时并切成 24 段、每段 1 小时。卡片通过率颜色跟随最右侧（当前）轨迹格子的健康状态；分组的通过率、轨迹和状态徽标都使用最新账户聚合检查，账户状态仍按真实请求、渠道错误和恢复探测的证据优先级计算。
