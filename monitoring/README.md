...
- 最近 24 小时的 24 段状态轨迹，每段 1 小时（分组使用成员账户聚合检查，账户可使用恢复探测补证据）。面板健康颜色只有三种：绿色表示可用，黄色表示仍可用但阶段性限速或延迟偏高，红色表示当前没有可用最终结果；没有证据的空档沿用最近有效状态并标记来源。429 尝试率与最终请求成功率分开统计，重试后成功不会被算成用户失败。
- 模型、分组的 Tokens 与成本分布分析；每百万 Tokens 成本用于总览、账户/分组卡片及趋势分析；
- 账户、分组用量卡片可切换展示四类 Tokens、原始/实际成本、有效倍率、缓存命中率与每百万 Tokens 实际成本，并支持按使用成本、每百万 Tokens 价格、有效倍率或总 Tokens 升序/降序排列，默认使用成本升序；
...
- 实时监控卡片显示近 5 分钟活跃用户数和有效请求数，并单独显示当前 5 分钟健康窗口：最终请求成功率、上游 429 尝试率、硬失败数和成功请求 P95；账户卡额外显示直接来自中转站 Redis 的当前并发请求数，分组卡不展示并发，避免共享账户造成归属误导；
