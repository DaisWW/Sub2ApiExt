...
- 模型、分组的 Tokens 与成本占比；
- 账户、分组的输入 Tokens、Cache Read 与缓存命中率，用于评估缓存收益；
...
支持 `1h`、`24h`、`today`、`7d`、`15d`、`30d`。返回启用账户/分组范围内的请求数、Tokens 总量、总成本、小时/天时间桶，以及账户、模型和分组排行；排行同时覆盖按 Tokens 与按成本的前列对象，账户和分组还附带缓存命中率。
