...
- 一次真实请求成功只证明分组当前存在可用路径；如果之后的账户巡检发现分组候选异常，面板会先显示“降级/待确认”，避免单次探测失败把仍可用的分组误报为整体失败。连续失败达到告警阈值后才显示“失败”；后续真实请求不会抹掉分组风险，仍需新的巡检结果确认恢复。
...
面板健康统计固定使用最近 24 小时，统一控制窗口观测通过率、首字中位数、最快、中位数、P95 和 24 段状态轨迹；状态徽标及卡片底部的“最近验证”则使用最新有效证据，用于反映当前状态。
