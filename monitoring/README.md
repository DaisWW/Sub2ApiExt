...
真实请求的首字指标使用 `usage_logs.first_token_ms`；主动探测显示的是 HTTP 首响应字节近似值，历史弹窗会明确区分。分组把 `ops_error_logs` 中按请求 ID 去重后的最终失败合并进窗口观测，账户把尚未恢复的渠道错误合并进窗口观测；两者都跳过中间 failover 行和业务限制错误，主动探测失败仍是独立账户信号。
