...
- 实时流量卡片：按最近 5 分钟有效请求统计活跃用户、渠道、账户及渠道到账户路由；该口径是“窗口内活跃”，不等同于进行中的请求数；
...
支持 `1h`、`24h`、`today`、`yesterday`、`7d`、`15d`、`30d`。返回启用账户/分组范围内的请求数、Tokens 总量、总成本、每百万 Tokens 成本、包含渠道明细的分钟/小时/天时间桶，以及账户、模型和分组排行。`1h` 使用分钟时间桶，`24h`、`today` 和 `yesterday` 使用小时时间桶，较长窗口使用天时间桶。`today` 为今天 00:00 到当前时刻，`yesterday` 为前一天 00:00 到今天 00:00。`limit` 是每个排行维度的上限；模型和分组数组是 Tokens、成本、每百万 Tokens 成本等各维度前列对象的去重并集，因此数组总数可能大于 `limit`。账户和分组还附带缓存命中率。
...
接口只返回最近 5 分钟有效请求的聚合数量：活跃用户、请求数、使用渠道、使用账户，以及渠道到账户的路由统计；各类明细按活跃用户数排序，每类最多返回 20 项，不会返回用户标识、API Key、IP 或会话明细。路由以 `usage_logs` 最终落库的 `channel_id` 和 `account_id` 为准，不统计未落库的重试尝试。同一用户在多个账户或渠道有请求时，会分别计入对应明细，因此明细人数之和可能高于全局活跃用户数。面板默认每 10 秒轮询该接口。它反映窗口内最近有请求的用户，不提供严格的进行中请求用户映射。
