...
Cockpit Tools 当前公开的外部深链只支持导入。增量总流程把新增账号和兑换结果标记为授权已更新的账号交给 Cockpit；输入中减少的账号会写入 `..\cache\results\cockpit-pending-deletions.txt` 供手动处理，工具不会直接修改 Cockpit 的加密账号存储。
