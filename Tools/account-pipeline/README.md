...
总入口 `run.bat` 启动 Python 编排器。完整流程的用户输入只有卡密 TXT；JSON 是兑换后的中间数据，也可以用于单模块重跑。编排器依次调用四个独立模块：
...
不传文件时读取 `input\redeem-codes.txt`；文件一行一个卡密，空行和 `#` 开头的行会忽略。拖入文件时只使用拖入文件。总入口不读取 JSON；需要直接使用 JSON 时，请运行对应的单模块 BAT。`--dry-run` 只验证本地布局，不访问兑换站或导入服务。
...
主流程的外部输入是卡密 TXT。Redeem 解压后，`normalize` 读取 `.json`/`.jsonl`，生成 `normalized\sub2api-accounts.json` 和 `normalized\cockpit-accounts.json`。这两个 JSON 是各自导入器的单独重跑输入，不是另一条默认的卡密来源。标准化要求每个账号有 `access_token` 和可识别的邮箱，并拒绝显式声明为非 `codex` 的记录。
