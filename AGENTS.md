...
- rate-sync 区分账号/分组模式、稳定 ID、水位、充值折扣和待发布目标；失败不猜测、不重复消费。
- rate-sync 账户配置只使用 `recharge_discounts`：值是充值折扣而非最终账户倍率（例如 `0.115 × 0.9 = 0.1035`）；未列出的域名默认折扣 `1.0` 并仍自动同步；禁止直接把 `0.9` 写成账户最终倍率。
