...
账户模式每轮优先读取 Sub2API `/v1/sub2api/billing` 返回的 `resolved_rate_multiplier`；该值是上游已经解析好的静态账号倍率。接口不可用时再兼容 NewAPI 的价格表和最新计费日志，所有直接倍率都不可用时才读取 `/v1/usage`。只要 `actual_cost / cost` 能算出有效正倍率，就作为账户候选倍率，不要求当前本地倍率为 `1.0`、最小成本或最小请求数；最终写回值仍会乘 `upstream_factors` 的上游系数。`effective_rate_multiplier` 可能包含峰值时段系数，不用于固化账户倍率。旧配置里的 `usage_bootstrap` 仍可读取，但不再改变此行为。
...
每轮同步完成后还会输出 `[TABLE]` 汇总表。账户 worker 的表格按“账号、账户倍率、预期倍率、结果、代理”显示，分组 worker 的表格只显示分组倍率；两张表分别按账号或分组去重，每个对象只显示一行。账户表的“预期倍率”是本轮候选上游倍率乘 `upstream_factors` 后的最终账户倍率，没有可用候选时显示 `-`；账户表中的代理地址会脱敏。分组表不显示代理，因为分组同步不直接使用代理，并包含独立的“说明”列；详细动态计算指标仍只出现在普通运行日志中。使用 `view-sync-logs.bat once` 或对应的 `account-once` 可以查看最近一轮的表格。

账户表的“结果”使用短分段显示状态、来源和必要的原倍率，例如 `检查｜自动探测`、`稳定｜上游同步`、`已更新｜请求计算｜原 0.1500`。`自动探测`表示 Sub2API billing，`上游同步`表示 NewAPI 价格表或计费日志，`请求计算`表示 `/v1/usage` 的实际成本比。系统优先读取 Sub2API 已解析倍率，其次兼容上游价格表或最新计费日志；只有直接倍率不可用时才回退到请求计算，因此来源由本轮证据决定，不按渠道名称固定。三种结果都会再乘 `upstream_factors`。`原` 表示本轮写回前的账户倍率。

两张表共用以下状态；其中“检查”的确认次数语义主要适用于账户 worker：

- `检查`：本轮请求和模板检查完成，但尚未形成可写回候选，例如确认次数未到或暂时没有有效请求成本。
- `稳定`：候选倍率与当前值一致，无需写回。
