...
账户表的“结果”会附带倍率来源：`上游同步` 表示直接读取上游价格表或最新计费日志中的倍率，`请求计算` 表示上游直接倍率不可用，改用 `actual_cost / cost` 计算；两种结果都会再乘 `upstream_factors`。

两张表共用以下状态；其中“检查成功”的确认次数语义主要适用于账户 worker：

- `检查成功`：本轮请求和模板检查成功，但候选倍率尚未达到确认次数。
- `稳定`：候选倍率与当前值一致，无需写回。
