...
每轮同步完成后还会输出 `[TABLE]` 汇总表。账户 worker 的表格只显示账号倍率，分组 worker 的表格只显示分组倍率；两张表分别按账号或分组去重，每个对象只显示一行。分组表不显示代理，因为分组同步不直接使用代理；账户表中的代理地址会脱敏。分组表包含独立的“说明”列，显示简短状态摘要；详细动态计算指标仍只出现在普通运行日志中。使用 `view-sync-logs.bat once` 或对应的 `account-once` 可以查看最近一轮的表格。

账户表的“结果”会附带倍率来源：`上游同步` 表示直接读取上游价格表或最新计费日志中的倍率，`请求计算` 表示上游直接倍率不可用，改用 `actual_cost / cost` 计算；两种结果都会再乘 `upstream_factors`。状态含义如下：`检查成功` 表示本轮请求和模板检查成功，但候选倍率尚未达到确认次数；`稳定` 表示候选倍率与当前值一致，无需写回；`预览` 表示计算出新值但 `dry_run` 阻止写回；`已更新` 表示 Admin API 已成功写入；`暂不自动` 表示为避免猜测而跳过，本轮保持原值；`失败` 表示请求、解析、计算或写回失败，保持原值；`未处理` 只表示该行尚未进入本轮处理流程。
