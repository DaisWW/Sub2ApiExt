...
> 重要：账户配置只维护 `recharge_discounts` 一个映射。它的值是“充值折扣”，不是最终账户倍率。未列出的域名按 `1.0` 计算，仍会自动同步。例如上游倍率 `0.115`、充值折扣 `0.9`（九折）时，最终写回 `0.1035`（`0.115 × 0.9`）。不能因为配置值为 `0.9` 就直接把账户倍率写成 `0.9`。折扣 `1.0` 不必写入映射。
...
账户 worker 的周期是 `900s`（15 分钟）。只要本轮得到有效的上游倍率，就立即按充值折扣计算并写回；不会再等待多轮确认。Lucen 和 TokenHorse 的上游倍率在写回前各自乘一次映射中的 `0.9`（覆盖 `lucen.cc`、`xixiapi.io` 和 `ppsubapi.com`）；分组 worker 不会再次乘该充值折扣。

账户模式每轮优先读取 Sub2API `/v1/sub2api/billing` 返回的 `resolved_rate_multiplier`；该值是上游已经解析好的静态账号倍率。接口不可用时再读取 NewAPI 的价格表和最新计费日志，所有直接倍率都不可用时才读取 `/v1/usage`。只要 `actual_cost / cost` 能算出有效正倍率，就作为账户候选倍率，不要求当前本地倍率为 `1.0`、最小成本或最小请求数；最终写回值仍会乘 `recharge_discounts` 中对应的充值折扣。`effective_rate_multiplier` 可能包含峰值时段系数，不用于固化账户倍率。
...
账户配置只支持 `recharge_discounts`；使用旧配置时需先将字段手动改为新名称，旧字段不会自动转换。
...
无法匹配模板时不会猜测，也不会修改倍率。NewAPI 渠道至少需要产生一条已计费的真实请求日志；日志会直接给出本次计费使用的价格组，避免把覆盖模型较多的高价组误判成当前组。每轮若无法读取或解析实时计费日志、日志倍率缺失，或日志中的价格组不在当前价格表中，本地倍率保持不动，并明确记录为同步失败；不会使用旧状态冒充本轮检查结果。分组 worker 不执行上游价格探测；以后遇到第三种稳定价格 API，只需在账户模式代码中新增一个模板适配器，无需扩展每渠道配置。
...
每轮同步完成后还会输出 `[TABLE]` 汇总表。账户 worker 的表格按“账号、账户倍率、预期倍率、充值折扣、结果、代理”显示，分组 worker 的表格只显示分组倍率；两张表分别按账号或分组去重，每个对象只显示一行。账户表的“预期倍率”是本轮候选上游倍率乘 `recharge_discounts` 中的充值折扣后的最终账户倍率；未列出的域名显示 `1.0000`，没有可用候选时预期倍率显示 `-`。账户表中的代理地址会脱敏。分组表不显示代理，因为分组同步不直接使用代理，并包含独立的“说明”列；详细动态计算指标仍只出现在普通运行日志中。使用 `view-sync-logs.bat once` 或对应的 `account-once` 可以查看最近一轮的表格。
