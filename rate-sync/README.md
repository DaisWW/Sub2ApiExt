...
账户表的“结果”使用短分段显示状态、来源和必要的原倍率，例如 `检查｜请求计算`、`稳定｜上游同步`、`已更新｜请求计算｜原 0.1500`。系统优先读取上游价格表或最新计费日志；只有直接倍率不可用时才回退到 `actual_cost / cost` 请求计算，因此来源由本轮证据决定，不按渠道名称固定。两种结果都会再乘 `upstream_factors`。`原` 表示本轮写回前的账户倍率。
