...
func (s *Syncer) publishDynamicGroup(
	...
	if err := s.store.Save(s.state); err != nil {
		...
		s.logger.Printf("[%s] 动态成本状态持久化失败，跳过更新 %.4f: %v", group.Name, target, err)
		return
	}
...
func (s *Syncer) retryDynamicPending(
	ctx context.Context,
	group *sub2APIGroup,
	state *DynamicGroupState,
	reason string,
	...
	target := state.PendingTarget
	if !validPositiveRate(target) {
		clearDynamicPending(state)
		report.markGroup(group.ID, reportStatusSkipped)
		report.setGroupEvidence(group.ID, reason, "待发布目标无效，等待重新初始化")
		return
	}
	report.setGroupEvidence(group.ID, reason, fmt.Sprintf("待发布目标=%.4f", target))
	if !dynamicGroupRateChangeSignificant(group.RateMultiplier, target) {
		clearDynamicPending(state)
		report.markGroup(group.ID, reportStatusStable)
		return
	}
	if s.config.DryRun {
		clearDynamicPending(state)
		report.markGroup(group.ID, reportStatusPreview)
		s.logger.Printf("[%s] dry-run 动态成本待发布目标: 当前 %.4f -> %.4f，不执行更新", group.Name, group.RateMultiplier, target)
		return
	}
	if err := s.store.Save(s.state); err != nil {
		report.markGroup(group.ID, reportStatusFailed)
		s.logger.Printf("[%s] 动态成本状态持久化失败，跳过待发布更新 %.4f: %v", group.Name, target, err)
		return
	}
	if err := s.updateGroup(ctx, group, target); err != nil {
		report.markGroup(group.ID, reportStatusFailed)
		s.logger.Printf("[%s] 动态成本待发布更新失败，稍后重试 %.4f: %v", group.Name, target, err)
		return
	}
	clearDynamicPending(state)
	report.updateGroupRate(group.ID, target)
	report.markGroup(group.ID, reportStatusUpdated)
	s.logger.Printf("[%s] 已重试动态成本待发布目标: 当前 %.4f -> %.4f", group.Name, group.RateMultiplier, target)
}
